...
app.get('/api/prezemi',(req,res)=>{
   ...
   if(smoking!='')
    ...
    if(internet!='')
    {
        nizaReq.push(['internet!','']);
    }
    if(invalidiziranost!='')
    {
        nizaReq.push(['wheelchair',invalidiziranost]);
    }
    if(phone!=''){
        nizaReq.push(['phone!','']);
    }
    if(nacinPlata!=undefined)
    {
        console.log(nacinPlata+"--------");
        if(nacinPlata.includes('Mastercard'))
        {
             nizaReq.push(["mastercard","yes"]);
        }
        if(nacinPlata.includes('Cash'))
        {
             nizaReq.push(["coins","yes"]);
        }
        if(nacinPlata.includes('Viza'))
        {
             nizaReq.push(["visa","yes"]);
        }
